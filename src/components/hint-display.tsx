  ...
  if (guessesAndHints.length === 0) {
    return (
      <div className="rounded-lg border border-border bg-panel p-8 text-center">
        <p className="text-muted-foreground">Make your first guess to see hints about the target KOL</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-3">
      {guessesAndHints.map((item, guessIndex) => (
        <div key={guessIndex} className="rounded-lg border border-border bg-panel p-4 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-semibold">{item.guess.name}</p>
              <p className="text-sm text-muted-foreground">{item.guess.twitterHandle}</p>
            </div>
            <span className="text-sm text-muted-foreground">Guess {guessIndex + 1}</span>
          </div>

          <div className="grid grid-cols-5 gap-2">
            {item.hints.map((hint, hintIndex) => {
              const bgColor =
                hint.type === "correct" ? "bg-[#10b981]" : hint.type === "close" ? "bg-[#f59e0b]" : "bg-[#4b5563]"
